...
namespace TexDrawLib
{
    ...
    public class CharBox : Box
    {
        ...
        public static CharBox Get(TexStyle style, TexCharMetric Char, FontStyle fontStyle)
        {
            ...
            if (ch.font.type == TexFontType.Font && ch.supported) {
                CharacterInfo c;
                if (DrawingContext.GetCharInfo(ch.fontIndex, ch.characterIndex,
                            (int)(TexUtility.RenderTextureSize * Char.appliedScale) + 1, fontStyle, out c)) {
                    float ratio = Char.appliedScale / (float)(c.size == 0 ? TexUtility.RenderTextureSize : c.size);
