...
namespace TexDrawLib
{
    public class TexTMPImporter
    {
        ...
        public static void SetupGUI (TexFont font, GUILayoutOption maxWidth)
        {
            ...
            _bufferWidth = EditorGUILayout.IntPopup(_bufferWidth, FontResolutionLabels, FontAtlasResolutions, GUILayout.ExpandWidth(true)); //, GUILayout.Width(80));
            _bufferHeight = EditorGUILayout.IntPopup(_bufferHeight, FontResolutionLabels, FontAtlasResolutions, GUILayout.ExpandWidth(true)); //, GUILayout.Width(80));

            GUILayout.EndHorizontal();

            GUILayout.BeginHorizontal(maxWidth);
            GUILayout.Label("Padding", GUILayout.Width(EditorGUIUtility.labelWidth));
            _padding = EditorGUILayout.IntSlider(_padding, 1, 10, maxWidth);
            GUILayout.EndHorizontal();

            _render = (RenderModes)EditorGUILayout.EnumPopup(_render, maxWidth);
            if (GUILayout.Button("Render", maxWidth))
            {
                CreateSDFAsset(font);
            }
            EditorGUI.BeginDisabledGroup(onRendering || !font.SDF_Asset);
            GUI.backgroundColor = Color.red;
            if (GUILayout.Button("Delete SDF Asset", maxWidth))
            {
            ...
            EditorGUI.EndDisabledGroup();
            {
                ...
                if (GUILayout.Button("Render All Fonts", maxWidth))
                {
                ...
                if (GUILayout.Button("Delete All SDF Asset", maxWidth))
                {
            ...
            if (onRendering)
            {
                ...
                EditorGUI.ProgressBar(EditorGUILayout.GetControlRect(maxWidth), prog, prog.ToString("P"));
                if (onRenderingBatch > 0) {
                    if (onRenderingBatch == 3)
                        EditorGUILayout.HelpBox("Will be stopped after current rendering is done", MessageType.Info);
                    else {
                         GUI.backgroundColor = Color.yellow;
                        if (GUILayout.Button("Cancel", maxWidth))
                            onRenderingBatch = 3;
                         GUI.backgroundColor = Color.white;
                    }
                }
