...
public class TEXBoxHighlighting
{
    ...
    static GUIStyle style = new GUIStyle(EditorStyles.textArea);
    static GUIStyle styleBg = new GUIStyle(GUI.skin.label);
    ...
    public static void DrawText(SerializedProperty prop)
    {
        ...
        prop.stringValue = DrawText(prop.stringValue);
    }
    ...
    public static string DrawText(string text, float realHeight, params GUILayoutOption[] options)
    {
        ...
        if (typ == EventType.ExecuteCommand)
        {
            ...
            if (ev.character == '\t' || ev.keyCode == KeyCode.Tab) {
    ...
    static float CalculateHeight(GUIContent content, GUIStyle style, int maxLineNumber, out float realHeight)
    {
        realHeight = EditorStyles.textArea.CalcHeight(
            content, EditorGUIUtility.currentViewWidth - 15);
		 int num2 = Mathf.Clamp(Mathf.CeilToInt(realHeight / 13f), 3, maxLineNumber);
        return (32f + ((num2 - 1) * 13));
    }

	static float scroll;
    static void HandleScrolling(Rect r, TextEditor editor, Event ev, GUIStyle style)
    {
    ...
    static bool HandleExecuteCommands(TextEditor editor, Event ev)
    {
        string commandName = ev.commandName;
        switch (commandName)
        {
    ...
    static int FindBeforeGroup(string text, int pos)
    {
        pos--;
    ...
    static int FindAfterGroup(string text, int pos)
    {
        if (text.Length <= 0 || pos < 0 || pos >= text.Length)
            return text.Length;
        var group = 0;
