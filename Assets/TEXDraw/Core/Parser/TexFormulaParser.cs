...
namespace TexDrawLib
{
	public partial class TexFormulaParser
	{
		...
		{
			//Look for and extract the escape command
//            var result = new StringBuilder();
			...
			var command = value.Substring (startPosition, position - startPosition);
...

			{
				...
				for (int i = 0; i < command.Length; i++) {
					var charAtom = CharAtom.Get (command [i], 
									TEXPreference.main.defaultTypefaces [TexCharKind.Commands]);
					row.Add (charAtom);
				}
		...
		private void RegisterTexFont (TexFormula formula, string value, ref int position, TexFont font)
		{
			...
			if (value [position] == leftBracketChar) {
				style = ParseFontStyle (ReadGroup (formula, value, ref position, leftBracketChar, rightBracketChar));
				SkipWhiteSpace (value, ref position);
			}
