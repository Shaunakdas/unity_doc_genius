...
{
    // For locating symbols to TexChar (using hash)
    [Serializable]
    public class TexSymbolDictionary : SerializableDictionary<string, int>
    {
#if UNITY_EDITOR
        public TexSymbolDictionary()
            : base ()
        {
            warnOnSetIfExist = true;
        }
        protected override void ThrowDuplicateKey(string key, int newValue)
        {
            throw new ArgumentException(string.Format("Symbol is already exists: {0}, with old hash {1:X3}, and new hash {2:X3}", key, this[key], newValue));
        }
#endif
    }

    // For locating math punctuation to TexChar (using hash)
    [Serializable]
    public class TexCharMapDictionary : SerializableDictionary<char, int>
    {
#if UNITY_EDITOR
        public TexCharMapDictionary()
            : base()
        {
            warnOnSetIfExist = true;
        }
        protected override void ThrowDuplicateKey(char key, int newValue)
        {
            throw new ArgumentException(string.Format("Character map for \'{0}\' is already exists, with old hash {1:X3}, and new hash {2:X3}", key, this[key], newValue));
        }
#endif
    }

    // For saving project-wide config saved in pref
    [Serializable]
    public class TexPrefsDictionary : SerializableDictionary<string, float>
    {
    }

    // Given a char in specific TexFont, result the index to targeted TexChar (in array index)
    [Serializable]
    public class TexCharTranslatorDictionary : SerializableDictionary<char, int>
    {
        
    }

    //No More make it a serialized dictionary, for faster performance
    [Serializable]
    ...
    public class SerializableDictionary<TKey, TValue> : IDictionary<TKey, TValue>
    {
        ...
        public bool warnOnSetIfExist = false;
        ...
        public TValue this[TKey key]
        {
            ...
            set { Insert(key, value, warnOnSetIfExist); }
        }
        ...
        public void Clear()
        {
            ...
            {
                // Something went wrong. Corrupt data?
                Initialize(_Count);
            }
        ...
        protected virtual void ThrowDuplicateKey(TKey key, TValue newValue)
        {
            throw new ArgumentException(string.Format("Key already exists: {0}, with old value {1}, and new value {2}", key, this[key], newValue));
        }
