...
namespace TexDrawLib
{
	public class AttrTransformationAtom : Atom
	{
		...
		static readonly Matrix4x4 _identity = Matrix4x4.identity;
		...
		const string _TParseIdentity = "XYZ";
		...
		static Vector3 SubParseTransformation (ref int pos, string value, int mode)
		{
			var n = mode == 2 ? 1f : 0f;
			if (pos >= value.Length)
		...
		static Vector3 GetVectorParsingCode (int code, float size, bool scale)
		{
			var n = scale ? 1f : 0f;
			switch (code) {
			//case 0:
