    ...
    public class DrawingParams
    {
        ...
        public void Render()
        {
            ...
            for (int i = 0; i < formulas.Count; i++)
            {
                ...
                // Get ready to render
                float x = offset.x + alignX * (sizeX - box.Scale * box.Box.width);
        ...
        {
            var useWordWrap = (int)autoWrap >= 2;
            var realBlankSpaceWidth = TexUtility.spaceWidth;
            size.x = 0;
            int i = 0;
            ...
            while (i < formulas.Count)
            {
                ...
                if (!(box is HorizontalBox && (box.width + formulas[i].PenaltyWidth) * scale > rectArea.width))
                {
                    i++;
                    continue;
                }
                ...
                for (int j = 0; j < box.children.Count; j++)
                {
                    ...
                    if (child is StrutBox && ((StrutBox)child).policy == StrutPolicy.BlankSpace)
                    {
                        ...
                        child.width = realBlankSpaceWidth; // All spaces must have this width (they may modified after WordWarpJusified).
                     }
                     ...
                     if (useWordWrap && lastSpaceIdx >= 0)
                    {
                        ...
                        if (autoWrap == Wrapping.WordWrapJustified && spaceIdxs.Count > 1)
                        {
                    ...
                    m_formulas[i].partOfPreviousLine = (useWordWrap && lastSpaceIdx >= 0) ? 2 : 1;
        ...
        {
            var count = verts.currentVertCount;
            switch (autoFill)
            {
