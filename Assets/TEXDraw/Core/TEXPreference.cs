...
#region Runtime Utilities
        ...
        public TexCharMetric GetCharMetric(char ch, TexStyle style)
        {
            ...
            if (ch >= 'A' && ch <= 'Z') // char.IsUpper(ch)
                font = fontData[defaultTypefaces[TexCharKind.Capitals]];
            else if (ch >= 'a' && ch <= 'z') // char.IsLower(ch)
                font = fontData[defaultTypefaces[TexCharKind.Small]];
            else if (ch >= '0' && ch <= '9') // char.IsDigit(ch)
                font = fontData[defaultTypefaces[TexCharKind.Numbers]];
            else
                font = fontData[(int)preferences["DefaultMuFontID"]];
