...
namespace TexDrawLib
{
	...
	public class TexMetaRenderer : IFlushable
	{
		...
		static string[] longTokens = {"style", "font", "lead", "left", "right", "kern", "line", "space", "para", "size", "align"};
		static char[] shortTokens = { 't'	 , 'f'	 , 'l'	 , 'b'	 , 'r'	  , 'k'	  , 'h'   , 'n'	   , 'p'   , 's'   , 'x'	};
		static char[] styleTokens = {'b', 'i', 'u', 's'};
		...
		public void ApplyBeforeBoxing () {
            if (!enabled)
                return;
			if (style != -1)
                TexUtility.RenderFontStyle = (FontStyle)(style); //Only Bold or Italic
            if (font != -2)
                TexUtility.RenderFont = font;
            if (kerning != 0)
                TexUtility.AdditionalGlueSpace = kerning;
		}
		...
		void ProcessToken (int tokenIdx, string raw, ref int pos) {
			ProcessToken(tokenIdx, raw, ref pos, tokenIdx == 0 ? styleTokens : (tokenIdx == 10 ? alignTokens : null));
		}
		...
		void ProcessToken (int tokenIdx, string raw, ref int pos, char[] presetTokens) {
			...
			switch (tokenIdx)
			{
				// "style", "font", "lead", "left", "right", "kern", "space", "para", "size", "align"
